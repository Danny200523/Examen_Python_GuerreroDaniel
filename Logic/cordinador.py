...
def abrirJSON():
    dicFinal={}
    with open('Data/Servicios.json','r') as openFile:
        dicFinal=json.load(openFile)
    return dicFinal

def guardarJSON(dic):
    with open("Data/Servicios.json",'w') as outFile:
        json.dump(dic,outFile)

def abrirJSON2():
    dicFinal={}
    with open('Data/Usuarios.json','r') as openFile:
        dicFinal=json.load(openFile)
    return dicFinal

def guardarJSON2(dic):
    with open("Data/Usuarios.json",'w') as outFile:
        json.dump(dic,outFile)
    ...
    for i in range(len(usuarios)):
        if usuarios[i]["id"]==id:
            ...
            if opcion==1:
                ...
                usuarios[i]["servicio en uso"]=input("Ingrese el nuevo servicio:")
            else:
...
def modificarServicio():
    ...
    for i in range(len(servicios)):
        if servicios[i]["name"]==nombre:
            ...
            opcion=int(input("Ingrese la opcion a modificar:"))
            if opcion==1:
                servicios[i]["name"]=input("Ingrese el nuevo nombre:")
            elif opcion==2:
                servicios[i]["description"]=input("Ingrese la nueva descripcion:")
            elif opcion==3:
                servicios[i]["price"]=int(input("Ingrese el nuevo precio:"))
            else:
                print("Opcion invalida")
            guardarJSON(servicios)
...
def verServicios():
    servicios=abrirJSON()
    for i in range(len(servicios)):
        print("Nombre:",servicios[i]["name"])
        print("Descripcion:",servicios[i]["description"])
        print("Precio:",servicios[i]["price"])
        print("")
